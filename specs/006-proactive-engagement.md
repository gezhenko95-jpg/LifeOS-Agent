...
**Текущая дата в промпте.** Системный промпт (`_system_prompt()`, не
константа — пересобирается на каждый вызов) начинается с `"Сегодня
YYYY-MM-DD."`. Без этого модель не знает, какой сейчас год, и может
додумать `target_date` в прошлом (реальный найденный баг — цель с
дедлайном в 2023 году). Тот же фикс — в `ai_fallback.py::_system_prompt`
для `due_date`.

**Прозрачность при "не понял".** Если `action` не распознан
(`None`/`unrelated`, либо распознан, но без обязательного поля) **и**
вопрос был задан недавно (`_UNANSWERED_PROMPT_NOTE_WINDOW = 30 минут` в
`engine.py`) — к обычному ответу (например, «Добавил задачу: …»)
дописывается пояснение: `"(Не понял это как ответ на «…» — если хотел
ответить, напиши ещё раз.)"`. Без этого пользователь не понимает, почему
его ответ на вопрос бота превратился в случайную задачу (см. историю с
«Сохрани лес» → задача без объяснений). Если вопрос уже старый —
пояснение не добавляется, чтобы не раздражать напоминанием о вопросе,
на который пользователь и не пытался отвечать.
