...
Прод: коммит `b87a762`, `bash lifeos-agent/scripts/deploy.sh --check`
...
4. **pytest запускать только из `lifeos-agent/`.** Из корня репозитория
   ломаются импорты и получается 530 «падений» на пустом месте.
5. Практика scratch-проверки миграций соблюдена для 015, 016 и 017.
