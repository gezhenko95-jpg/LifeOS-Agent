...
разделов и приведено к одному правилу для всех кнопок, и написан разбор
`MULTIUSER.md` — что нужно, чтобы пустить в бота посторонних людей.

Прод: коммит `49a9845`, `bash lifeos-agent/scripts/deploy.sh --check`
зелёный, 772 теста.
...
- Ответить на приглашение бота можно и голосом.

### 3. `MULTIUSER.md` — разбор перехода к продукту
...
└── lifeos-agent/
    ...
    └── tests/digest/, tests/telegram/test_menu_navigation.py  (NEW)
```

Живой проверки владельцем пока не подтверждено: голосовой ввод (висит с
прошлой сессии) и новые экраны меню. После обновления клиента нужно
отправить боту `/menu` — ReplyKeyboardMarkup живёт на устройстве, пока
её не заменят, иначе новые кнопки не появятся.
